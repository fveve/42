...
/*   Updated: 2024/06/25 15:42:00 by arafa            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sleep(t_philo *philo, char *msg, long sleeping_time)
{
	...
	while ((convert_time() - time) < sleeping_time && !ft_death(philo) && !ft_check(philo->param))
		usleep(15);
}
...
int	ft_death(t_philo *philo)
{
	if (!ft_check(philo->param) && (convert_time() - philo->time_end) >= philo->dying_time && !ft_check_everyone(philo->param))
	{
...
int	check_fork(t_philo *philo)
{
	...
	{
		if (ft_check(philo->param) || ft_check_everyone(philo->param) || ft_take_fork(philo))
		{
			pthread_mutex_unlock(&philo->param->death);
			return(1);
		}
		else if (philo->has_eaten == philo->nb_of_dining)
		{
			...
			philo->has_eaten = -1;
		//	printf("philo->param->everyone_has_eaten : %d\n", philo->param->everyone_has_eaten);
			return(1);
		}
